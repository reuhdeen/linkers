...
import {
  ...
  FaPlus,
  FaMinus,
} from "react-icons/fa"; // Import FaPlus and FaMinus
import debitCredit from "./debitcredit.png";
import CartModal from "../api/cartModal"; // Ensure the correct path
import ReceiptModal from "../api/receiptModal"; // Ensure the correct path
import CategorySidebarNav from "../components/categorySideBar"; // adjust path if needed
            ...
            {loading ? (
              ...
              filteredProducts.map((product) => {
                ...
                return (
                  ...
                  >
                    <div className="card menu-card position-relative">
                      {decodeBase64(product.discount_status) === "active" && (
                        <span className="discount-badge">
                          {product.discount_percent * 100}% OFF
                        </span>
                      )}
                      <img
                        src={`/product_images/${product.productID}.png`}
                ...
                orders.map((order) => (
                  ...
                  >
                    <div className="row align-items-center">
                      ...
                      {/* B1-C1: Name and Quantity Controls */}
                      <div className="col-7">
                        <div className="fw-bold">{order.name}</div>
                      ...
                      <div className="col-3 text-right">
                        ...
                        <button
                          className="btn btn-sm custom-btn mt-2"
                          onClick={() => handleDeleteOrder(order.id)}
                        >
                          <FaTrashAlt size={16} />
                        </button>
              ...
              <div className="mt-3">
                ...
                >
                  {showPaymentOptions ? "Place Order" : "Continue to Payment"}
                      ...
                      <div className={`number-keyboard-container ${activeInput ? 'visible' : ''}`}>
        <NumberKeyboard onKeyPress={handleKeyPress} />
      </div>
          </div>
        </div>
